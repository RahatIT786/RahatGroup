...
class UmrahPackage extends Component
{
    ...
    public function fetchPackages()
    {
        ...
        $baseQuery = Packages::where('is_active', 1)
            ...
            ->when(!empty($this->selectedMonth), function ($query) use ($month_pkg) {
                $query->whereIn('id', $month_pkg->unique());
            })
        ...
        $this->packages = $baseQuery
            ...
            ->filter(function ($package) {
                // Filter packages that have at least one pkgDetail with g_share in the specified range
                return $package->pkgDetails->contains(function ($detail) {
                    return $detail->g_share >= $this->minPrice && $detail->g_share <= $this->maxPrice;
                });
            })
            ->sortBy(function ($package) {
                // Sort by the minimum g_share value from the pkgDetails relationship
    ...
    {
        return view('user-front.pages.umrah-package', [
