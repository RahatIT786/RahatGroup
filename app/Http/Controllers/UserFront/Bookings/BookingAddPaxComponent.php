...
class BookingAddPaxComponent extends Component
{
    ...
    public function onPassportDocumentChange($index)
    {
        ...
        if ($file) {
            ...
            if ($data && isset($data['extracted_data'])) {
                ...
                $expiryDate = $extractedData['date_of_expiry'];

                $parsedDob = !empty($dob) ? Carbon::createFromFormat('d/m/Y', $dob)->format('Y-m-d') : null;
                $parsedExpiry = !empty($dob) ? Carbon::createFromFormat('d/m/Y', $expiryDate)->format('Y-m-d') : null;
                ...
                // $this->passengerDetails[$index]['age'] = $extractedData['age'] ?? '';
                // Add other fields as necessary
                // logger("Passport reader extractedData : ", $extractedData);
                // logger("Passport reader passengerDetails : ", $this->passengerDetails[$index]);
            } else {
                // Handle error or missing data
                $this->addError('passport_front', 'Failed to extract data from passport.');
            }
        } else {
            $this->addError('passport_front', 'No passport front image uploaded.');
        }
    }
    ...
    {
        return view('user-front.bookings.booking-add-pax-component');
    }
}
