...
use App\Mail\HajjPackagesInquiryEmail;

class PackageViewModalComponent extends Component
{
    use WithFileUploads;
    ...
    public $children ,$adults ,$infants;
    ...
    // protected $listeners = ['travelDateUpdated' => 'refreshTravelDate'];

    // public function refreshTravelDate($date)
    // {
    //     $this->travel_date = $date;
    // }

    public function mount($id, $travel_date, $city, $airline_id , $selectedPackageFlavourId)
    {
        // dd($selectedPackageFlavourId);
        // dd($this->city_id);
        $this->packageId = $id;
        // Access session variables
        // $this->pkg_flavour_id = Cache::get('pkg_flavour');
        $this->pkg_flavour_id = $selectedPackageFlavourId;
        // dd($this->pkg_flavour_id);
    ...
    public function generateCaptcha()
    {
        ...
        imagedestroy($image);
        session(['book_now_hajj_captcha_code' => $text]);
    }
    ...
    public function save()
    {
        ...
        if ($this->userInput !== session('book_now_hajj_captcha_code')) {
        ...
        if ($validated) {
            $hajjpackages = Bookingenquiry::create($validated);
            // Mail::to($randomStaff->email)->cc('joddhajitputel143@gmail.com')->send(new HajjPackagesInquiryEmail($hajjpackages, $randomStaff));
    ...
    {
        return view('agent.packages.package-details.package-view-modal-component');
    }
}
