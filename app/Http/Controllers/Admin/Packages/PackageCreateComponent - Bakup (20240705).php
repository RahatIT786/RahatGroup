...
class PackageCreateComponent extends Component
{
    ...
    public $package_includes = [], $package_type_ids;
    ...
    {
        //   dd($this->package_name,$this->package_type_ids,$this->service_id);

    }

    public function getMakkaHotel()
    {   
        // dd($this->makka_rating);
        foreach ($this->makka_rating as $pkgCard_id => $rating) {
            $this->makka_hotel[$pkgCard_id] = HotelMaster::where('star_rating', $rating)
                ->where('city_id', 1)
                ->pluck('hotel_name', 'id');
        }
        //  dd($this->makka_rating,$this->makka_hotel);
    }

    public function getMadinaHotel()
    {   
       
        foreach ($this->madina_rating as $pkgCard_id => $rating) {
           
            $this->madina_hotel[$pkgCard_id] = HotelMaster::where('star_rating', $rating)
                ->where('city_id', 2)
                ->pluck('hotel_name', 'id');
        }
        // dd($this->madina_rating,$this->madina_hotel);
    }

    // public function getModalContent($selectedValue, $dropdown)
    // {   
    //     dd($selectedValue, $dropdown);
