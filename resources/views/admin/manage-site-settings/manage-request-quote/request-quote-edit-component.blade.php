...
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>{{ __('tablevars.edit') }} {{ __('tablevars.quote') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active">{{ __('tablevars.ManageSiteSettings') }}</div>
                <div class="breadcrumb-item"><a href="{{ route('admin.request.index') }}" wire:navigate>{{ __('tablevars.quote') }} {{ __('tablevars.list') }}</a></div>
                ...
                <div class="card">
                    <form wire:submit.prevent="update">
                        <div class="card-header d-flex justify-content-between">
                            <h4>{{ __('tablevars.edit') }} {{ __('tablevars.quote') }}</h4>
                            <a href="{{ route('admin.request.index') }}" class="btn btn-danger" wire:navigate><i class="fas fa-long-arrow-alt-left"></i> &nbsp;{{ __('tablevars.back') }}</a>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-6">
                                    ...
                                    <select class="form-control" wire:model.lazy="pkg_type_id" id="pkg_type_id" disabled >
    <option value="">Select package type</option>
    @foreach ($packagemaster as $id => $package_name)
        <option value="{{ $id }}">{{ $package_name }}</option>
    @endforeach
</select>
@error('pkg_type_id') <span class="text-danger">{{ $message }}</span> @enderror
                                    </div>
                                </div>
