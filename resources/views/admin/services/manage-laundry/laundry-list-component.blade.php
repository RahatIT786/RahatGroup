<div class="main-content">
    ...
    <section class="section">
        <div class="section-header">
            <h1> {{ __('tablevars.manage') }} {{ __('tablevars.laundry') }}</h1>
            <div class="section-header-button">

            </div>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active">{{ __('tablevars.manage') }} {{ __('tablevars.laundry') }}
                </div>
                <div class="breadcrumb-item"><a href="{{ route('admin.laundry.index') }}"
                        wire:navigate>{{ __('tablevars.manage') }} {{ __('tablevars.laundry') }}</a></div>
                            ...
                            <div class="row">
                                <div class="col-3">
                                    ...
                                    <input type="text" class="form-control" wire:model='search_name'
                                        placeholder="Search Name" wire:keyup="filterLaundry">
                                </div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <tbody>
                                        @forelse ($laundries as $key => $laundry)
                                            <tr>
                                                ...
                                                {{-- <td>{{ $laundry->booking_date }}</td> --}}
                                                {{-- <td>{{ $laundry->no_of_guest }}</td> --}}
                                                <td>{{ $laundry->name }}</td>
                                                <td>{{ $laundry->email }}</td>
                                                <td>{{ $laundry->mobile }}</td>
                                                {{-- <td>{{ $laundry->whatsapp }}</td> --}}
                                                {{-- <td>{{ $laundry->hotel_name }}</td> --}}
                                                {{-- <td>{{ $laundry->comments }}</td> --}}
                                                <td>
                                                    @if ($laundry->staffmaster)
                                                        {{ $laundry->staffmaster->first_name . ' ' . $laundry->staffmaster->last_name }}
                                                    @else
                                                        {{ '-' }}
                                                    @endif
                                                </td>
                                                ...
                                                @endphp
                                                    ...
                                                    @if (array_key_exists($laundry->status, $statusLabels))
                                                        <div
                                                            class="badge {{ $statusLabels[$laundry->status]['class'] }}">
                                                    ...
                                                    <div class="btn-group mb-2">
                                                        ...
                                                        <div class="dropdown-menu" x-placement="bottom-start"
                                                            ...
                                                            <a href="javascript:void(0)" data-bs-toggle="modal"
                                                                ...
                                                                wire:click="getModalContent({{ $laundry->id }})">View</a>
                                                            {{-- <a class="dropdown-item"
                                                                href="{{ route('admin.laundry.edit', $laundry->id) }}"
                                                                data-toggle="tooltip"
                                                            ...
                                                            <a href="javascript:void(0)"
                                                                ...
                                                                data-toggle="tooltip"
                                                                wire:click='isDelete({{ $laundry->id }})'>{{ __('tablevars.trash') }}</a>
                                                            <a href="javascript:void(0)" data-bs-toggle="modal"
                                                                class="dropdown-item"
                                                                data-bs-target="#relationshipModal"
                                                                data-toggle="tooltip"
                                                                title="Assign Relationship Manager"
                                                                wire:click="getModalRelationship({{ $laundry->id }})">Assign
